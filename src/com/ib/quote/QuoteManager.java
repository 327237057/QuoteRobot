...
public class QuoteManager {
    ...
    public double getTradeBidPrice(){
        synchronized(QUOTEACCESSLOCK){
            LOG.debug("Returning trade bid price = " + tradeBidPrice);
    ...
    public double getTradeAskPrice(){
        synchronized(QUOTEACCESSLOCK){
            LOG.debug("Returning trade ask price = " + tradeAskPrice);
    ...
    private void fetchStaticOffset(){
        if(staticOffset == Double.MAX_VALUE){
            staticOffset = Double.parseDouble(ConfigFrame.getInstance().getStaticOffsetField());
        }
    }
    
    private void fetchDefaultSpread(){
        if(defaultSpread == Double.MAX_VALUE){
            defaultSpread = Double.parseDouble(ConfigFrame.getInstance().getDefaultSpreadField());
        }
    }
    ...
    private void fetchSourceExchange(){
        if(sourceExchange == null){
            sourceExchange = ConfigFrame.getInstance().getSourceExchangeField();
        }
    }
