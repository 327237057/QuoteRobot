...
public class IBClient implements EWrapper {
    ...
    private AtomicBoolean pauseExecution = new AtomicBoolean(false);
    
    private OrderTableModel orderTableModel = OrderTableModel.getInstance();
        ...
        if(orderId > 0){ // Only cares about API orders
            ...
            if(status.equalsIgnoreCase("Cancelled")){
                ...
                synchronized(Trader.ORDERCANCELMONITORLOCKFORWRAPPER){
                    ...
                    orderTableModel.removeTableRow(orderId);
    ...
    public void error(int id, int errorCode, String errorMsg) {
        ...
        if((errorCode == 201 || errorCode == 202 || errorCode == 10147 || errorCode == 10148)&& m_cancelHandler.pendingCancelListContains(id)){
            ...
            synchronized(Trader.ORDERCANCELMONITORLOCKFORWRAPPER){
                ...
                orderTableModel.removeTableRow(id);
                Trader.ORDERCANCELMONITORLOCKFORWRAPPER.notifyAll();
                LOG.debug("Received order cancel status, notifying Cancel Handler.");
            }
        }
    ...
    public void updateAllConfigs(){
        ...
        OrderTableModel.getInstance().updateAllConfigs();
        m_orderHandler.updateAllConfigs();
        m_positionManager.updateALlConfigs();
        m_positionMonitor.updateAllConfigs();
        m_quoteManager.updateAllConfigs();
    }
}
