...
public class OrderHandler implements Runnable{
    ...
    private void updateCurrentBuyOrder(int orderId){
        ...
        try{
            ...
            if(!m_cancelHandler.pendingCancelListContains(orderId)){
                ...
                LOG.debug("Current buy order price = " + currentBuyOrder.lmtPrice() + " vs. tradeBidPrice = " + tradeBidPrice);
    ...
    private void updateCurrentSellOrder(int orderId){
        ...
        try{
            ...
            if(!m_cancelHandler.pendingCancelListContains(orderId)){
                ...
                LOG.debug("Current sell order price = " + currentSellOrder.lmtPrice() + " vs. tradeSellPrice = " + tradeAskPrice);
    ...
    public Contract getTradeContract(){
    ...
    private void fetchTradeExchange(){
        if(tradeExchange == null){
            tradeExchange = ConfigFrame.getInstance().getTradeExchangeField();
        }
    }
    
    private void fetchOrderSizeDefault(){
        if(orderSizeDefault == Integer.MAX_VALUE){
            orderSizeDefault = Integer.parseInt(ConfigFrame.getInstance().getOrderSizeDefaultField());
        }
    }
    
    private void fetchPositionAdjustment(){
        if(positionAdjustment == Integer.MAX_VALUE){
            positionAdjustment = Integer.parseInt(ConfigFrame.getInstance().getPositionAdjustField());
        }
    }
    
    private void fetchAccount(){
        if(account == null){
            account = ConfigFrame.getInstance().getAccountField();
        }
    }
    ...
    private void checkPauseExecution(){
        while(m_client.getPauseExecution()){
            try{
                //LOG.debug("Program is paused, wait for 10 seconds.");
                Thread.sleep(1000);
            } catch (Exception e){
                LOG.debug(e.getMessage(), e);
            }
